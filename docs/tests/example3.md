...
Само приложение это простой инструмент для ведения некоторых сообщений и посылке их другим пользователям.
...
*/lab3/db/dump/* */lab3/db/load/* соответственно. При запуске веб сервера восстановление происходит автоматически.

В тесте мы переопределили инициализирующие методы, указанные ранее.
При запуске, данные сохраняются и база очищается. Так же при каждом новом тесте, методе, хранилище заново очищается.
...
Но нам еще необходимо проверить валидность данных введенных в сами формы. Для этого мы выделили метод *assertFieldErrors*.
...
Что бы удостоверится что сообщение отображается в списке в списке, проверим количество записей. А так же текст и отправителя.
...
Например такую как посылка сообщение самому себе. И многое другое.
