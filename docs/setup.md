...
Далее необходим web-браузер Firefox для запуска самих тестов. Если он у вас еще не установлен,
...
Запустите файл *run.bat*. Появится терминал с нотификацией, что запущен web сервер.
...
Там же можно поменять путь до файла, порт и хост web сервера.